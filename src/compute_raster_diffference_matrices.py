...
def compute_raster_difference_matrices(rasters_list: list[tuple[str, int]], reclass_spec: PixelRemapSpecs, *, status: Status | None = None, status_prefix: str = '') -> tuple[list[str], list[pandas.DataFrame]]:
  ...
  # function to calculate difference matrix for a given year gap
  def calculate_difference_matrix(year_gap: int) -> None:
    for (index, (file_this_year, year)) in enumerate(rasters_list):
      # get data for the previous year
      file_last_year, last_year = next((t for t in rasters_list if t[1] == year - year_gap), (None, None))
  ...
  # calculate difference matrices for different year gaps
  calculate_difference_matrix(1)  # one year gap
  calculate_difference_matrix(3)  # three year gap
  calculate_difference_matrix(5)  # five year gap
  calculate_difference_matrix(len(rasters_list) - 1)  # entire year range
